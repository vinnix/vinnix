...
## ######################################################################################
## Oracle Cheat Sheet - Helping DBAs Around the World
## 
...
BEGIN
  FOR object_info IN
    (SELECT owner,
      table_name
    FROM dba_tables
    WHERE lower(owner) IN ('datawhare')
    AND TEMPORARY       = 'N'
...
-- -------------------------------------------------------------------------------------
create user &&NEW_USERNAME identified by supersecrettestpasswordexample123;
...
BEGIN
  ...
  FROM DBA_ROLES
  WHERE ROLE LIKE 'RL%TEST%SEL'
  OR ROLE LIKE 'RL%TEST%SEL'
  )
  LOOP
...
-- --------------------------------------------------------------------------------------
$ impdp schema_to_backup CONTENT=DATA_ONLY DIRECTORY=DUMP_DIRECTORY DUMPFILE=schema_to_backup.dmp log=impdp_20151104.log
-- --------------------------------------------------------------------------------------
...
-- IndeXES STats
-- --------------------------------------------------------------------------------------
create index IDXDM_T01_DATA_ABERTURA_DESC on dmbio_t01(DATA_ABERTURA desc) online TABLESPACE INDEX_DW_CORE;

begin 
    for rr in ( select * from user_indexes where index_name like 'IDXDM_%' ) 
    loop
            DBMS_STATS.GATHER_INDEX_STATS ( OWNNAME=>'schema'
...
create materialized view MV_DMBI_FATO as select * from dw_core.DMBI_T01 mwhere rownum <=0;

declare
begin
        dbms_snapshot.refresh('MV_DMBI_FATO', 'F');
exception
       when others then
...
CREATE MATERIALIZED VIEW mv_bi_min_max
BUILD IMMEDIATE
REFRESH FAST
...
AS
   SELECT  T01A.bank
           T01A.agency
           T01A.account
           MIN(T01A.value_predicted) min_value,
           MAX(T01A.value_predicted) max_value
    FROM warehouse.data_mart_T01 T01A
    GROUP BY T01A.bank
      T01A.agency,
      T01A.account
;
...
-- --------------------------------------------------------------------------------------
    delete from datamart_prod where rowid in ( 
        select to_del_rowid
          from (
                select taux.* 
                      , row_number() over ( partition by  tab01_id, tab02_id, tab03_id  order by tab01_id, tab02_id ) as numpos
                from (
                      select   pais_id, prodh_id, hosp_id
                             , count(*)  over (partition by tab02_id, tab02_id, tab03_id order by tab01_id, tab02_id ) as count1
...
>> prod-br.db.cloud.domain.com.br [ x.y.z.p ] [N. Virginia] → Primary 
>> dataguard.db.cloud.domain.com.br [j.k.z.p] [SP] (dataguard) → Physical Standby

[ref.:http://www.oracle.com/technetwork/articles/sql/11g-dataguard-083323.html ]

Abrir DG para escrita!
-- ------------------------------------
ssh ec2-user@dataguard.db.cloud.domain.com.br

$ dgmgrl /

> convert database schema01 to snapshot standby;

---
sqlplus / as sysdba

> update schema.usuarios set email = '-', password='1234';
> commit;

> update schema.usuarios set email = '-';
> commit;

> alter trigger schema01.user_trigger disable;
> update schema01.user_trigger set usr_email='-', usr_senha = '1234';

> update schema.usuarios set CRYPTPASSWORD = null;
...
$ dgmgrl sys/dataguardpasswd@dataguard
...
#producao_brasil =
#(DESCRIPTION = (ADDRESS = (PROTOCOL = TCP)(HOST = hostname-address.domain.com)(PORT = 1521)) (CONNECT_DATA = (SERVER = DEDICATED) (service_name = prodbr)))
...
Analyzing tablespaces...
  ...
  WARNING: Imminent error in host prd.db.cloud.domain.com.br:srv_administrator with tablespace DATA_AUTO [ 3% free space - 40239.19 mb of 1343488 mb ]

  WARNING: Imminent error in host prd.db.cloud.domain.com.br:srv_administrator with tablespace TBS_INDEX [ 12.29% free space - 3021.13 mb of 24576 mb ]

  WARNING: Imminent error in host prd.db.cloud.domain.com.br:srv_administrator with tablespace TBS_DATA [ 17.68% free space - 29691.13 mb of 167936 mb ]

Done

[16:43:20] oracle@prd:monitoring $ sqlplus / as sysdba
...
SQL> alter tablespace TBS_DATA add datafile '+DATA' size 100m autoextend on next 100m maxsize 4096m;
...
emcli login -username=sysman -password=anypassword

emcli delete_target -name="apex-br.db.cloud.domain:3872" -type="oracle_emd" -delete_monitored_targets -async

https://cloudcontrol.db.cloud.domain.com.br:7802/em/login.jsp
...
>> http://monitoring-nv.cloud.domain.com.br/prod/check_mk/

>> Monitoramento Check_Mk
http://monitoring.cloud.domain.com.br/prod


tail -F /u01/app/oracle/diag/rdbms/apex/apex/trace/alert_apex.log
tail -F /u01/app/oracle/diag/rdbms/biodwprd/biodwprd/trace/alert_biodwprd.log
...
-- Profiles
-- -----------------------------------------------------------------------------------------
alter profile domainbr_profile limit SESSIONS_PER_USER 200;
...
SELECT /*+ MATERIALIZE NO_MERGE */
 ...
 WHERE con.constraint_type = 'R'
   AND con.owner NOT IN ('ANONYMOUS','APEX_0','APEX_1','APEX_SSO','APPQOSSYS','CTXSYS','DBSNMP','DIP','EXFSYS','FLOWS_FILES','MDSYS','OLAPSYS','ORACLE_OCM','ORDDATA','ORDPLUGINS','ORDSYS','OUTLN','OWBSYS')
   ...
   and con.owner in ('OWNER01','OWNER02', 'OWNER03','CAT', 'DOC')
...
select * from sys.dba_tab_modifications where table_owner = 'data_warehouse';
...
-- -----------------------------------------------------------------------------------------
 ...
 where di.index_name = ds.segment_name
   and di.owner in ('owner01','owner02') 
...
-- -----------------------------------------------------------------------------------------
 ...
 $ impdp scott/tiger CONTENT=DATA_ONLY DIRECTORY=DATA_PUMP  DUMPFILE=dump_file.dmp log=log_file.log EXCLUDE=TABLE:\"IN \(\'AUDIT_TABLE\',\'LOG_ACOES\',\'EMAIL_FILA\'\) \"  table_exists_action=append  PARALLEL=1
